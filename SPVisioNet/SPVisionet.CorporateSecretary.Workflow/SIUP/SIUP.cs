...
{
    public sealed partial class SIUP : SequentialWorkflowActivity
    {
        ...
        //public static void CreateErrorLog(SPWeb web, Exception ex, string ListName)
        //{
        //    using (SPSite site = new SPSite(web.Url, web.Site.SystemAccount.UserToken))
        //    {
        //        SPWeb saWeb = site.OpenWeb();
        //        SPList list = saWeb.GetList(Util.CreateSharePointListStrUrl(saWeb.Url, "ErrorLog"));
        //        saWeb.AllowUnsafeUpdates = true;

        //        SPListItem listItem = list.Items.Add();
        //        listItem["Title"] = ListName;
        //        listItem["Message"] = ex.Message;
        //        listItem["Source"] = ex.Source;
        //        listItem["TargetSite"] = ex.TargetSite;
        //        listItem["StackTrace"] = ex.StackTrace;

        //        if (ex.InnerException != null)
        //            listItem["InnerException"] = ex.InnerException.Message;

        //        listItem.Update();

        //        saWeb.AllowUnsafeUpdates = false;
        //    }
        //}
        ...
        private void onWorkflowActivated1_Invoked(object sender, ExternalDataEventArgs e)
        {
            ...
            WFNameID = Util.GetSettingValue(web, "Workflow Name", "SIUP");
            ...
            //catch (Exception ex)
            //{
            //    CreateErrorLog(workflowProperties.Web, ex, "SIUP");
            //}
        ...
        private void PopulateDeptHead_ExecuteCode(object sender, EventArgs e)
        {
            ...
            createMultipleTaskDeptHead_Subject1 = string.Format("CorsecSP {0} [ {1} ] Need Approval", RequestCode.SIUP, workflowProperties.Item["NamaPerusahaan"].ToString());
            createMultipleTaskDeptHead_Body1 = string.Format(EmailNotificationTemplate, "{0}", "SIUP Task", workflowProperties.Item.Title, OriginatorName, "need your approval" + Util.GenerateApprovalInformation(scInfo), "{1}");
        ...
        public StringDictionary sendEmailOriginator_Headers1 = new System.Collections.Specialized.StringDictionary();
        ...
        private void sendEmailOriginator_MethodInvoking(object sender, EventArgs e)
        {
            ...
            string URL = string.Format("{0}/Lists/SIUP/EditForm.aspx?ID={1}", workflowProperties.WebUrl, workflowProperties.Item.ID);
            Subject = string.Format("CorsecSP {0} [ {1} ] Rejected", RequestCode.SIUP, workflowProperties.Item["NamaPerusahaan"].ToString());

            BodyMessage = string.Format(ApprovedRejectedMailTemplate, OriginatorName, "SIUP", workflowProperties.Item.Title, "has been rejected by " + DivHeadName + Util.GenerateApprovalInformation(scInfo) + "<br/><br/>Remarks: " + Remarks, URL);
        ...
        private void PopulateDataOriginator_ExecuteCode(object sender, EventArgs e)
        {
            ...
            createMultipleTaskOriginator_Subject1 = string.Format("CorsecSP {0} [ {1} ] Need to Upload Document and Update Data", RequestCode.SIUP, workflowProperties.Item["NamaPerusahaan"].ToString());
            createMultipleTaskOriginator_Body1 = string.Format(EmailNotificationOriginatorTemplate, "{0}", "SIUP Task", workflowProperties.Item.Title, "need you to upload document and update data" + Util.GenerateApprovalInformation(scInfo), "{1}");
        ...
        private void SendMailDeptHead_ExecuteCode(object sender, EventArgs e)
        {
            ...
            string URL = string.Format("{0}/Lists/SIUP/DispForm.aspx?ID={1}", workflowProperties.WebUrl, workflowProperties.Item.ID);
            Subject = string.Format("CorsecSP {0} [ {1} ] is already Completed", RequestCode.SIUP, workflowProperties.Item["NamaPerusahaan"].ToString());
            ...
            foreach (string item in splitDivHead)
            {
                ...
                BodyMessage = string.Format(ApprovedRejectedMailTemplate, userDivHead.Name, "SIUP", workflowProperties.Item.Title, "has been uploaded document and updated data by " + OriginatorName + Util.GenerateApprovalInformation(scInfo), URL);
